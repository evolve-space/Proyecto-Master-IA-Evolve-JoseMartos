...
class OutlookOAuthController extends AbstractController
{
    ...
    public function connect(MicrosoftGraphOAuthService $oauthService): JsonResponse
    {
        try {
            $auth = $oauthService->createAuthorizationUrl();
    ...
    public function callback(Request $request, MicrosoftGraphOAuthService $oauthService): RedirectResponse
    {
        ...
        if (is_string($error) && $error !== '') {
            ...
            return new RedirectResponse($frontend.'/correos?outlook=error&message='.rawurlencode($description));
        }

        $state = $request->query->get('state');
        if (!$oauthService->validateState(is_string($state) ? $state : null)) {
        ...
        if (!is_string($code) || $code === '') {
            return new RedirectResponse($frontend.'/correos?outlook=error&message='.rawurlencode('Microsoft no devolvio codigo de autorizacion.'));
        }

        try {
            $oauthService->exchangeAuthorizationCode($code);
        } catch (\Throwable $e) {
            return new RedirectResponse($frontend.'/correos?outlook=error&message='.rawurlencode($e->getMessage()));
        }

        return new RedirectResponse($frontend.'/correos?outlook=connected');
    }
